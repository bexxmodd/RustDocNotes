...
* `cargo build` : to build a project. use `--release` to build the final program ready for production.
...
* `println!` : is a macro that prints a string on the screen.

* `let` statement is used to create a variable. By default variables in Rust are immutable. To make variable mutable add `mut`.

For example, let's create a mutable variable that will be a string.
```
...
Remember that a crate is a collection of Rust source code files. To use outside the library we need to add it to `Crate.toml` under the `[dependencies]`.
...
* `match` : expression is made of _arms_. An arm consists of a pattern and the code that should be run if the value given to the beginning of the `match` expression fits that arm's pattern.

When we get the input from the user we can convert it to any type we want. For example, if we want to convert to unsigned 32-bit number:
```
...
`guess` refers to the original value of that variable (that was a string). 

* `trim` : is a method on a string instance that will eliminate any white spaces at the beginning and end.

* `parse` : method on strings parses a string into some kind of number. Because it can parse into a variety of numbers we need to tell exactly what type.
...
Constants aren't just immutable by default (meaning you can't use `mut` with them) - they are always immutable.
...
The naming convention for constants is to use all uppercase with underscores. Here's an example:
```
...
By using `let`, we can perform a few transformations on value but have the variable be immutable after those transformations have been completed.
...
If negative it needs to have signed so it will be _signed_ integer. Otherwise, it's _unsigned_.

The default integer type is `i32` which is recommended when you are not sure which type to use.

**FLoating-Point Types** also has two types, which are numbers with decimal points. We can choose the bits and are denoted with `f` --> Example: `f32`.
the default type is `f64` which is recommended.

**The Boolean Type** is only bite in size. It's specified using `bool`.

**Character Type** is called with `char` and is the language's most primitive alphabetic type and they use single quotes opposed to String types that use double-quotes.
...
Because a tuple is a single compound element. To get the individual values out of a tuple, we can use pattern matching to _destructure_ a tuple value:
```bash
fn main() {
    let tup = (500, 6.1, 4);
...
**Array** is another way to have multiple values in a collection. Unlike a tuple, every element in the tuple should have the same type.
They also have fixed length like tuples. Values are written in array as comma-separated list: `let a = [2, 3, 1, 7]`.

Arrays are useful when you want your data allocated on the stack rather than the heap.
A vector is a similar collection type provided by the standard library that is allowed to grow or shrink in size.

A good example of when to use array rather than a vector is in a program that needs to know the name of the months of the year.
...
a function definition in Rust starts with `fn`.
...
_Statements_ are instructions that perform some action and do not return a value. Function definitions are also statements.
_Expressions_ evaluate to a resulting value. Expressions do not include semicolons. If you add a semicolon at the end you'll turn it into a statement.
...
You can leave comments by starting with two forward slashes -> `// Some comment``. Comments only continue until the end of the line.
...
The next one is the conditional loop `while` which has a very familiar syntax: `while i < 0` etc.
You could use the `while` to the loop of the elements of a collection, such as an array.
...
Ownership features do not slow down your program.
...
The _stack_ stores value in the order it gets them and removes the values in the opposite order. Also referred to as _last in, first out_.
Adding data is called pushing onto the stack and removing data is called popping off the stack.
All data stored on the stack must have know, fixed size. Data with an unknown size at a compile-time or a size that might change must be stored on the heap instead.
...
A string is stored on the heap. You can create a `String` from a string literal using the `from` function:
```bash
...
Rust automatically calls `drop` function for us after closing the curly bracket.
...
This group takes a look at Figure 4-1 to see what is happening to String under the covers. A String is made up of three parts, shown on the left:
...
When we assign `1` to `s2` `String` data is copied, meaning we copy the pointer, the length, the capacity that is on the stack.
...
Rust also invalidates the first variable, instead of being called a shallow copy, it's known as _move_. With the above example, we would say that `s1` was _moved_ into `s2`.
...
Because we have a reference, not a copy, where `&s1` refers to `s1`, `s1` will not be dropped if the reference goes out of scope.
...
The benefit of having this restriction is that Rust can prevent data races at compile time. A _data race_ is similar to race conditions and happens when:
* Two or more pointers access the same data at the same time.
* At least one of the pointers is being used to write to the data.
* There’s no mechanism being used to synchronize access to the data.

If we try to return a reference to the string with the function we'll get an error, because the string will be dropped when it goes out of scope (end of the function)
and reference is pointing to a none existing string. In that case, you need to return the actual string instead of the reference.
...
if you start from the 0 indexes you can drop zero and use it like this: `let slice = &s[..2];`. Similarly no need to indicate the last index value.
